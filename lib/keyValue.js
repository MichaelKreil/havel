...
module.exports = Havel => {
	...
	Havel.registerNode('bufferToKeyValue', () => Havel.wrapFunc({
		func: async function* bufferToKeyValue(iterator) {
			for await (const buffer of iterator) {
				yield new KeyValue(buffer);
			}
			this.finish();
		},
		inputType: 'buffer',
	...
	Havel.registerNode('keyValueToBuffer', (maxParallel) => {
	...
	Havel.registerNode('keyValueCheckOrder', (cbCheck) => Havel.wrapFunc({
		func: async function* keyValueCheckOrder(iterator) {
			...
			for await (let entry of iterator) {
				...
				if (lastKey !== undefined) {
					if (!(await cbCheck(lastKey, key))) {
						this.error(`Error in keyValueCheckOrder. Order violates given rule: …, ${lastKey}, ${key}, …`);
					}
				}
				lastKey = key;
				yield entry;
	...
	Havel.registerNode('streamToKeyValue', () => Havel.compose(p =>
		p.streamToBoxes().bufferToKeyValue()
	))

	Havel.registerNode('keyValueToStream', (maxParallel) => Havel.compose(p =>
		p.keyValueToBuffer(maxParallel).boxesToStream()
	))

	Havel.registerNode('keyValueMerge', (pipeline, cbSort, cbEntry) => Havel.wrapFunc({
		func: async function* keyValueMerge(iterator1) {
			if (!(pipeline instanceof Havel.HavelPipeline)) throw Error('Havel.keyValueMerge needs a pipeline as first parameter')
			...
			let iterator2 = pipeline.iterator();
