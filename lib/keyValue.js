...
module.exports = Havel => {
	...
	async function* bufferToKeyValue() {
		for await (const buffer of await this.iterator) yield new KeyValue(buffer);
	}
	Havel.registerNodeFactoryFunction(bufferToKeyValue)

	async function* keyValueToBuffer (maxParallel) {
	...
	Havel.registerNodeFactoryFunction(keyValueToBuffer)

	async function* keyValueCheckOrder(cbCheck, cbError) {
	...
	Havel.registerNodeFactoryFunction(keyValueCheckOrder)
	...
	async function* keyValueMerge(pipeline2, cbSort, cbEntry) {
		...
		let iterator1 = await this.iterator;
	...
	Havel.registerNodeFactoryFunction(keyValueMerge)
