...
class HavelNodeFactory {
	...
	create(pipeline, args) {
		let node;
		try {
			//console.log('create', this.name, this.func);
			return node = {
				name: this.name,
				pipeline,
...
class HavelPipeline {
	 ...
	 */
	#add(factory, args) {
		//console.log('pipeline.#add', factory.name);
...
function registerNodeFactoryStream(name, func) {
	registerNodeFactory(name, async function () {
		//console.log(`init stream "${name}"`, func.name);
		...
		async function wrapWritableStream() {
			...
			writable.on('error', err => {
				//console.log('on.error', name, err);
				doContinue(); error = true
			})
			writable.on('drain', () => {
				//console.log('on.drain', name);
				doContinue()
			});
			for await (let chunk of iterator) {
				//console.log('writable.write', name);
