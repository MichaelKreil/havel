...
function Transform(opt = {}) {
	...
	const stream = new Stream.Duplex({
		...
		readable: true,
		writeable: true,
		readableObjectMode:  opt.inputType !== 'stream',
		writableObjectMode: opt.outputType !== 'stream',
		readableHighWaterMark: ( opt.inputType === 'stream') ? 1024*1024 : 16,
		writableHighWaterMark: (opt.outputType === 'stream') ? 1024*1024 : 16,
	});
