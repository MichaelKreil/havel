...
module.exports = Havel => {
	...
	function readFile(filename, opt) {
		...
		return Havel.wrapStream(stream, { inputType:'nothing', outputType:'stream', name:`readFile (${filename})` });
	}

	function writeFile(filename) {
		const stream = fs.createWriteStream(filename);
		return Havel.wrapStream(stream, { inputType:'stream', outputType:'nothing', name:`writeFile (${filename})` });
	}
}
