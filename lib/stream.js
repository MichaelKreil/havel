...
module.exports = havel => {
	...
	function readFile(filename, opt) {
		...
		return havel(stream, { inputType:'nothing', outputType:'stream', name:`readFile (${filename})` });
	}

	function writeFile(filename) {
		const stream = fs.createWriteStream(filename);
		return havel(stream, { inputType:'stream', outputType:'nothing', name:`writeFile (${filename})` });
	}
}
